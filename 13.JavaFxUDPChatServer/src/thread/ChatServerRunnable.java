			...
			try {
				...
				if(!isExist){ // 목록에 존재하지 않는 사용자라면..
					Platform.runLater(new Runnable() {
						...
						@Override
						public void run() {	// address는 inetAdress 객체
							clientList.add(address.getHostName()); // JavaFx Application 쓰레드 관련 작업임.
						}
					});
					
					ClientVO vo = new ClientVO(
							address.getHostAddress(), port, new String(inPacket.getData()).trim());
							// 대화명에서 불필요한 공백 제거.
					...
					while(it.hasNext()) {
						...
						// 보낼 데이터 설정. TCP의 sendToAll 과 똑같다.
