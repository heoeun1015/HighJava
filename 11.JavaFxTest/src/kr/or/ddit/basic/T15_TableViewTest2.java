...
public class T15_TableViewTest2 extends Application{
	...
	public void start(Stage primaryStage) throws Exception {
		...
		ObservableList<Member> data = FXCollections.observableArrayList(
				...
				new Member("혼길서", "gilseo", "33", "3333-3333", "대구"),
		...
		// 해당 컬럼에 나타날 데이터 연결하기
		// (출력할 객체의 메머변수와 출력할 컬럼을 매칭시킨다.)
		korNameCol.setCellValueFactory(new PropertyValueFactory<>("korName"));
		
		TableColumn<Member, String> engName = new TableColumn<>("영어이름");
		engName.setCellValueFactory(new PropertyValueFactory<>("engName"));
		...
		ageCol.setCellValueFactory(new PropertyValueFactory<>("tel"));
		
		TableColumn<Member, String> addrCol = new TableColumn<>("주소");
		ageCol.setCellValueFactory(new PropertyValueFactory<>("addr"));
	...
	public class Member{
		...
		public Member(String korName, String engName, int age, String tel, String addr) {
			super();
			this.korName = korName;	// 한글 이름
			this.engName = engName; // 영어 이름
			this.age = age;			// 나이
			this.tel = tel;			// 전화번호
			this.addr = addr;		// 주소
		}
