...
public class T15_ObjectStreamTest {
	public static void main(String[] args) {
		...
		try {
			...
			// FileOutputStream oos2 = new FileOutputStream("d:/D_Other/memObj.bin");
				// 이렇게도 가능은 한데 기능 향상을 위해 위처럼 붙여줬다.
			...
			oos.writeObject(mem4);	// 직렬화 처리
				// 직렬화 할 수 있는 인스턴스가 아니면 컴파일이 아니라 실행할 때 오류가 날 것.
...
/*───────────────────────────────────────────────────────────────────────────────────────────────────*/
			...
			//e.printStackTrace();	// 발생하는 예외를 그대로 출력. 더 이상 읽을 파일이 없다는 오류가 뜬다.
...
class Member implements Serializable{
	...
	// 구현을 하지 않으면 직렬화 작업시 java.io.NotSerializableException 예외가 발생한다.
	...
	private String name;
	private transient int age;	// 직렬화 대상에서 제외하고 싶을 때 사용. 값을 불러올 수 없다.
	private String addr;
