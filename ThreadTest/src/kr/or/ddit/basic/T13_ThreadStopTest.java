package kr.or.ddit.basic;
	...
	/*	Thread의 stop()메서드를 호출하면 쓰레드가 바로 멈춘다.
		이때, 사용하던 자원을 정리하지 못하고 프로그램이 종료되기 때문에 나중에 실행되는 프로그램에 영향을 줄 수 있다.
		그래서 현재는 stop()메서드는 사용하지 않는 것이 좋다.	*/
...
//		th.stop();
		...
		th2.interrupt();	// interrupt() 메서드 호출. 거는 순간 true, break 로 빠져나온다.
...
/*───────────────────────────────────────────────────────────────────────────────────────────────────*/
		...
		// 			interrup()메서드를 호출하면 InterruptedException이 발생한다.
...
/*───────────────────────────────────────────────────────────────────────────────────────────────────*/
		...
		while(true) {
			...
			if(this.isInterrupted()) {	// interrupt()메서드가 호출되면		//나 지금 인터럽트 걸렸냐
				System.out.println("- 인스턴스용 isInterrupted()");
				break;
			}
			
			// 검사방법2 → 쓰레드의 정적 메서드를 이용하는 방법
//			if(Thread.interrupted()) {	// interrupt()가 호출되면 true, 그 다음부터는 false
