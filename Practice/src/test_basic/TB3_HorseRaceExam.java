...
public class TB3_HorseRaceExam {
...
class Horse extends Thread implements Comparable<Horse> {
	...
	public void run() {
		for (int i = 0; i < 50; i++) { 
			try {
				Thread.sleep((int) (Math.random() * 200)); // 0~49 까지의 난수 발생(구간 사이의 딜레이를 주기 위함)
			} catch (InterruptedException e) {
				e.printStackTrace();
			}
			setHorsePosi(i); // 각 구간으로 말의 위치 이동
		}
		this.horseRank = TB3_HorseRaceExam.rank;
		TB3_HorseRaceExam.rank++;
	}
}
...
class HorsePositionDisplay extends Thread {
	...
	public void run() {
		...
		while (true) {
			...
			System.out.println("==========================================================");
			System.out.println();
			for (int i = 0; i < TB3_HorseRaceExam.hList.size(); i++) {
				if (TB3_HorseRaceExam.hList.get(i).getHorsePosi() != 49) {
					System.out.print(TB3_HorseRaceExam.hList.get(i).getHorseName() + " : ");
					System.out.print(horseCourse.substring(0, TB3_HorseRaceExam.hList.get(i).getHorsePosi()) + ">");
					System.out.println(horseCourse.substring(TB3_HorseRaceExam.hList.get(i).getHorsePosi(), 49));
				} else {
					System.out.print(TB3_HorseRaceExam.hList.get(i).getHorseName() + " : ");
					System.out.print(horseCourse.substring(0, TB3_HorseRaceExam.hList.get(i).getHorsePosi()) + "도착");
					System.out.println();
					finishedCnt++;
				}
			}
